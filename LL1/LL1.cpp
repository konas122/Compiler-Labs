...
// 提取左公共因子  找到一组“最大”左公共因子,因为node只有两个，一个产生式只有一种类型的左公共可以，有两个以上的还要改代码
void ex_com_le() {
    int n = all_gram.size();
...
//      int num_mcl;                            // 当前最小左公共因子的数量
        ...
        int *tag = (int *)malloc(sizeof(int) * (a1 + 2));
...
// 消除左递归,候选式中不会出现多个左递归，因为已经提取过公共左因子
void eli_le_rec(vector<grammer_node *> &all_gram) {
    int n = all_gram.size();
    for (int i = n - 1; i >= 0; i--) {  // 双重for循环把间接左递归转化为直接左递归
        ...
        for (int j = n - 1; j > i; j--) {
            ...
            //  int flag = 0;
            for (auto it = all_gram[i]->result.end(); it != all_gram[i]->result.begin();) {
                --it;
                if ((*it)[0] == all_gram[j]->non_ter_sym[0]) {
                    // cout << *it;
                    for (auto it1 = all_gram[j]->result.end(); it1 != all_gram[j]->result.begin();) {
                        --it1;
...
void parse(string expression) {
    ...
    cout << endl;
    cout << setiosflags(ios::left) << setw(17) << "步骤" << setw(18) << "已匹配" << setw(16)
         << "栈" << setw(18) << "输入串" << setw(17) << "动作" << '\n';
    ...
    while(analy_stack.size()) {
        if(analy_stack[0] == expression[cur_idx]) { // 如果栈顶和当前要分析的字符相同
            cout << setiosflags(ios::left) << setw(15) << step_numb << setw(15)
                 << expression.substr(0, cur_idx) << setw(15) << analy_stack << setw(15)
                 << expression.substr(cur_idx) << setw(15) << expression[cur_idx] << '\n';
            step_numb++;
            cur_idx++;
            ...
            // cout << static_cast<int>(analy_stack[0]) << " " << static_cast<int>(expression[cur_idx]) << endl;
            string str1;
            try {
                str1 = all_gram.at(i)->result.at(analy_table[analy_stack[0]][expression[cur_idx]]);
            }
            catch(...) {
                printf("错误: 非法的输入字符 %c, 跳过\n", expression[cur_idx]);
                cur_idx++;
                continue;
            }
            ...
            cout << setiosflags(ios::left) << setw(15) << step_numb << setw(15)
                 << expression.substr(0, cur_idx) << setw(15) << analy_stack << setw(15)
                 << expression.substr(cur_idx) << analy_stack[0] << "->" << str1 << '\n';
            step_numb++;
            analy_stack = analy_stack.substr(1);
            if (str1 != "@")
...
int main() {
    ...
    // 文法从文件里读入
    assert(freopen("../grammer.txt", "r", stdin) != NULL);
    char one_row[255] = {}; // 文件的一行，表示一个产生式
    char delims[] = " |>";
    char *str = NULL;

#ifdef _MSC_VER
    while (gets_s(one_row, sizeof(one_row)) != NULL)
